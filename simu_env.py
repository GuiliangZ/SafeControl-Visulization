...
#initiate obstacle that is adversirial
		cx,cy,_ = self.robot_state.position
		#self.field.adversarial_init(cx,cy)
		#self.field.stationary_set_init() # randomize the init position of obstacles
		self.field.random_init() # randomize the init position of obstacles
		
		most_dangerObs_ids = -1
		obs_t2_ids = -1

		obstacle_id, obstacle_pos, _ = self.find_nearest_obstacle(cx,cy, obs_t2_ids, most_dangerObs_ids)
	...
	def step(self, action, obs_t2_ids, most_dangerObs_ids, is_safe = False, unsafe_obstacle_ids = []):
		'''
		action: [dv_x, dv_y]
		'''
		self.cur_step += 1
		...
		nearest_obstacle_id, nearest_obstacle, collisions = self.find_nearest_obstacle(cx, cy, obs_t2_ids, most_dangerObs_ids, unsafe_obstacle_ids)
	...
	def find_unsafe_obstacles(self, min_dist):
		...
		for i, obs_state, obs_color in unsafe_obstacles:
			self.display.obstacle_at_loc(i, obs_state[0], obs_state[1])	
			if (i in unsafe_obstacle_ids):
				self.display.obstacle_set_color(i, obs_color)
		return unsafe_obstacle_ids, unsafe_obstacle_info, unsafe_obstacles


	def find_nearest_obstacle(self, cx, cy, obs_t2_ids, most_dangerObs_ids, unsafe_obstacle_ids = []):
		...
		collisions = ()
		for i,x,y,obs_color in astlocs:
			self.display.obstacle_at_loc(i,x,y)	
			if (i in unsafe_obstacle_ids):
				self.display.obstacle_set_color(i, obs_color)
#Change the color of the concerning most dangerous obstacle at t time step
			if (i == obs_t2_ids):
				self.display.obstacle_set_color(i, 'purple')
#Change the color of the AdamBA simulated concerning obstacle at t+1 time step
			if (i == most_dangerObs_ids):
				self.display.obstacle_set_color(i, 'red')
